...
function titleSearch(){
    ...
    if(titleInput !== '') {
        ...
        displayCatalog(arrayify(CID.where("Title", "==", titleInput)));
    }
}
...
function exclusiveSearchCatalog(){
    ...
    var completeArray = getWholeCatalog(CID, "asc").then(function(array){
        return array.filter(function(value){
            console.log("Current Value: " + value);
            var result = true;
            if(actorInput != ''){
                result = result && value["Actors"].includes(actorInput);
            }
            if(genreInput.length != 0){
            ...
            if(directorInput != ''){
                result = result && value["Director"] == directorInput;
            }
            if(ratingInput != ''){
